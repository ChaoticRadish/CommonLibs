using ChaoticKit.Data.Structure.Tree.Comparer;
...
namespace ChaoticKit.Data.Structure.Tree.Extensions
{
            ...
            where TNode : IAddableMultiTreeNode<TValue, TNode>, ISameTypeParentOwner<TNode>
        {
            ...
            for (int i = 1; i < arr.Length; i++) 
            {
                ...
                while (true)
                {
                    ...
                    else if (currentIncludeResult.HasFlag(TreeNodeIncludeCompareResult.DiffFork))
                    {
                        ...
                        if (currentChildEnumerator == null)
                        {
                            ...
                            if (currentNode.Parent == null)
                            {
                                ...
                                throw new ChaoticKit.Exceptions.General.ImpossibleForkException($"未能找到节点值 {value} 的非邻接父节点, 此节点值在分支 {firstValue} 之外");
                            }
                            else
                            {
                    ...
                    {
                        throw new ChaoticKit.Exceptions.General.ImpossibleForkException($"遍历过程是由浅层节点到深层节点遍历的, 不应该出现当前节点是节点值 {value} 的邻接子节点的情况! ");
                    }
                    else if (currentIncludeResult.HasFlag(TreeNodeIncludeCompareResult.DistantChild))
                    {
                        throw new ChaoticKit.Exceptions.General.ImpossibleForkException($"遍历过程是由浅层节点到深层节点遍历的, 不应该出现当前节点是节点值 {value} 的非邻接子节点的情况! ");
                    }
                    else
                    {
                        // 逻辑上没有其他的情况了
                        throw ChaoticKit.Exceptions.General.ImpossibleForkException.Create(currentIncludeResult);
                    }
                }
            Found:
