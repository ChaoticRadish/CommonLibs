...
namespace ChaoticKit.Data.Structure.Tree
{
    /// <summary>
    /// 通用树结构 (不定叉数的多叉树)
    /// </summary>
    ...
    public class GeneralTree<Scope, Value> : IMultiTree<Value?>
    {
        ...
        {
            return ChaoticKit.String.StringHelper.ConcatAndWrap(scopes.Select(i => i == null ? "<null>" : i.ToString()).ToList(), "->", "[", "]");
        }
        #endregion
            ...
            Func<GeneralTreeNode<Scope?, Value?>, string>? customGetTextLogic = null)
        {
            ...
            tempTree.Preorder((node, index) =>
            {
                ...
                builders[rowIndex]
                    .Append(leftStr)
                    .Append(ChaoticKit.String.StringHelper.FillBlankSpace(node.NodeValue.Text, layerStringMaxLength[node.NodeValue.LayerIndex]))
                    .Append(rightStr);
                // 其余行
                    ...
                    for (int i = 0; i < node.NodeValue.ChildrenEndPointCount; i++)
                    {
                        ...
                        else
                        {
                            builders[rowIndex + i].Append(
                                ChaoticKit.String.StringHelper.GetBlankSpaceString(
                                    splitHoz.Length
                                    + leftStr.Length
                    ...
                    for (int n = 0; n < node.Childrens.Count; n++)
                    {
                        ...
                        if (!node.NodeValue.IsEndPoint && node.NodeValue.ChildrenEndPointCount > 1)
                        {
                            for (int i = 1; i < childrenNode.NodeValue.ChildrenEndPointCount; i++)
                            {
                                ...
                                {
                                    builders[rowIndex + offset + i].Append(ChaoticKit.String.StringHelper.GetBlankSpaceString(splitVer));
                                }
                                else
                                {
