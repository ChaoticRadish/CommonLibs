...
using ChaoticKit.Data.Mechanisms.Extensions.Channel;
using ChaoticKit.Data.Struct;
using ChaoticKit.Extensions;
using ChaoticKit.Module.Reflection;
...
namespace ChaoticKit.Data.Mechanisms.Impl
{
    /// <summary>
    ...
    /// 处理的属性类型必须满足其中之一:  <br/>
    /// 1. 基础类型或实现 <see cref="ChaoticKit.Data.Constraint.IStringConveying{TSelf}"/> <br/> 的类型
        ...
        ) : IStringCodec, ICodec<string>, IStreamCodec<char>
    {
        ...
        {
            return ChaoticKit.Module.Config.ConfigStringHelper.Obj2ConfigValue(value) ?? throw new NullReferenceException("转换意外取得空字符串引用");
        }
        private static object _shortDataConvertFromString(TypeDetail typeDetail, string value)
        {
            return ChaoticKit.Module.Config.ConfigStringHelper.ConfigValue2Obj(value, typeDetail.UnderlyingType) ?? throw new NullReferenceException("转换意外取得空对象引用");
        }
