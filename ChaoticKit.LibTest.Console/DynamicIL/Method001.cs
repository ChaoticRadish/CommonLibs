...
using ChaoticKit.LibTest.Console.TestModels;

namespace ChaoticKit.LibTest.Console.DynamicIL
{
    public class Method001() : TestBase("动态生成代码 emit 实现统一调用某个方法")
    {
        protected override void RunImpl()
        {
            AssemblyName assemblyName = new AssemblyName("ChaoticKit.LibTest.Console.DynamicIL.ImplTest");
            ...
            foreach (MethodInfo method in methods)
            {
                ...
                {
                    ChaoticKit.Module.DynamicIL.CommonMethodLogic.UnityDealAsync(
                        iLGenerator, method,
                        dealHandlerAsync);
                }
                else
                {
                    ChaoticKit.Module.DynamicIL.CommonMethodLogic.UnityDeal(
                        iLGenerator, method,
                        dealHandler);
                }
            }
        ...
        public object? dealHandler(ChaoticKit.Module.DynamicIL.CommonMethodLogic.MethodInput input)
        {
        ...
        public async Task<object?> dealHandlerAsync(ChaoticKit.Module.DynamicIL.CommonMethodLogic.MethodInput input)
        {
            WriteLine("方法调用, 原型: " + input.Prototype.Name);
