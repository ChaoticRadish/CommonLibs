...
{
    internal class Analysis002() : TestBase("字符串解析测试 002, 测试 TryReadUntilOrEnd 寻找特定字符串")
    {
        ...
        protected override void RunImpl()
        {
            ...
            // 2. 找不到字符串的情况，返回范围内的所有字符
            RunTest("hello world", "abc", 0, null, true, "hello world", "2. 找不到字符串，返回全部");

            // 3. startIndex 为负数（调整为 0，maxReadLength 不变）
            RunTest("hello", "llo", -2, null, true, "he", "3. startIndex 为负数，maxReadLength 为 null");

            // 4. startIndex 为负数，maxReadLength 减少后不足以找到字符串，返回范围内的字符
            RunTest("hello", "llo", -2, 5, true, "hel", "4. startIndex 为负数，maxReadLength 减少后不足以找到字符串");

            // 5. startIndex 为负数，maxReadLength 减少后为负数
            RunTest("hello", "llo", -5, 3, true, "", "5. startIndex 为负数，maxReadLength 减少后为负数");

            // 6. startIndex 超过字符串长度，返回空字符串
            RunTest("hello", "o", 10, null, true, "", "6. startIndex 超出长度");

            // 7. maxReadLength 为 0，返回空字符串
            RunTest("hello", "h", 0, 0, true, "", "7. maxReadLength 为 0");

            // 8. maxReadLength 为负数，返回空字符串
            RunTest("hello", "h", 0, -1, true, "", "8. maxReadLength 为负数");

            // 9. maxReadLength 限制导致找不到，返回范围内的字符
            RunTest("hello world", "world", 0, 5, true, "hello", "9. maxReadLength 过小无法找到，返回范围内的字符");

            // 10. maxReadLength 刚好能找到
            RunTest("hello world", "world", 0, 11, true, "hello ", "10. maxReadLength 刚好足够");

            // 11. 空输入字符串
            RunTest("", "test", 0, null, true, "", "11. 空输入字符串");
            ...
            RunTest("hello world", "world", -3, 10, true, "hello w", "17. startIndex 负数与 maxReadLength 联动（缩减后不足）");
            ...
            // 21. maxReadLength 精确截断，返回范围内的字符
            RunTest("hello world", "world", 0, 6, true, "hello ", "21. maxReadLength 精确截断在匹配前，返回范围内的字符");
            ...
            // 24. 最大读取长度限制在匹配开始处，返回范围内的字符
            RunTest("hello world", "world", 0, 7, true, "hello w", "24. maxReadLength 终止在匹配开始处，返回范围内的字符");
            ...
            // 29. 最大读取长度刚好截断在查找字符串前，返回范围内的字符
            RunTest("123456", "456", 0, 3, true, "123", "29. 最大读取长度刚好截断在查找字符串前，返回范围内的字符");

            // 30. 最大读取长度包含部分查找字符串但不完整，返回范围内的字符
            RunTest("123456", "456", 0, 4, true, "1234", "30. 最大读取长度包含部分查找字符串但不完整，返回范围内的字符");

            // 31. startIndex 为负数，maxReadLength 减少后为 0，返回空字符串
            RunTest("hello", "llo", -3, 3, true, "", "31. startIndex 为负数，maxReadLength 减少后为 0");
            ...
            // 36. 边界情况：最大读取长度刚好在查找字符串开始处，返回范围内的字符
            RunTest("123456", "456", 0, 3, true, "123", "36. 最大读取长度刚好在查找字符串开始处，返回范围内的字符");

            // 37. 边界情况：最大读取长度包含查找字符串的一部分，返回范围内的字符
            RunTest("123456", "456", 0, 4, true, "1234", "37. 最大读取长度包含查找字符串的一部分，返回范围内的字符");

            // 38. 边界情况：最大读取长度刚好包含整个查找字符串
            RunTest("123456", "456", 0, 6, true, "123", "38. 最大读取长度刚好包含整个查找字符串");

            // 39. 特殊情况：maxReadLength 刚好截断在查找字符串中间，返回范围内的字符
            RunTest("123456789", "456", 0, 5, true, "12345", "39. maxReadLength 刚好截断在查找字符串中间，返回范围内的字符");

            // 40. 特殊情况：从中间开始查找，找不到时返回剩余部分
            RunTest("hello world", "abc", 6, null, true, "world", "40. 从中间开始查找，找不到时返回剩余部分");

            // 41. 特殊情况：maxReadLength 限制返回的部分
            RunTest("hello world", "abc", 6, 3, true, "wor", "41. maxReadLength 限制返回的部分");

            // 42. 特殊情况：查找字符串为空，返回空字符串
            RunTest("hello", "", 0, null, true, "", "42. 查找字符串为空，返回空字符串");

            // 43. 特殊情况：maxReadLength 为 null，不限制读取长度
            RunTest("hello world", "abc", 0, null, true, "hello world", "43. maxReadLength 为 null，不限制读取长度");

            // 44. 特殊情况：maxReadLength 大于字符串长度
            RunTest("hello", "x", 0, 10, true, "hello", "44. maxReadLength 大于字符串长度");

            // 45. 特殊情况：查找字符串在开头，startIndex 为 0
            RunTest("test123", "test", 0, null, true, "", "45. 查找字符串在开头，startIndex 为 0");

            // 46. 特殊情况：查找字符串在末尾，maxReadLength 刚好足够
            RunTest("123test", "test", 0, 7, true, "123", "46. 查找字符串在末尾，maxReadLength 刚好足够");
                              ...
                              bool expectedResult, string? expectedOutput, string description)
        {
            ...
            try
            {
                bool result = StringAnalysis.TryReadUntilOrEnd(input, findStr, startIndex, maxReadLength, out string? output);
