...
namespace ChaoticKit.Data.Constraint
{
    /// <summary>
    /// 与字符串可以互相无损转换的对象
    /// </summary>
    ...
    public static class StringConveyingHelper
    {
        ...
        private static string _toStr(Type type, object obj)
        {
            ...
            if (obj != null && (result == null || result is not string))
                throw new ChaoticKit.Exceptions.General.ImplementationException($"显示转换接口未按预期返回非 null 字符串值");
        ...
        private static object _toObj(Type type, string str)
        {
            ...
            if (str != null && (result == null || result.GetType() != type))
                throw new ChaoticKit.Exceptions.General.ImplementationException($"显示转换接口未按预期返回非 null 值");
            return result!;
        }
