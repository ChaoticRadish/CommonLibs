...
namespace ChaoticKit.Module.Reflection
{
    /// <summary>
    /// 元组相关的帮助类
    /// </summary>
    public static class ValueTupleHelper
    {
        ...
        {
            if (types.Length == 0)
                throw new ChaoticKit.Exceptions.General.ImplementationException("实现异常, 待整理为元组类型的类型列表为空");
            if (types.Length <= 7)
            {
                Type define = types.Length switch
                {
                    ...
                    6 => typeof(ValueTuple<,,,,,>),
                    7 => typeof(ValueTuple<,,,,,,>),
                    _ => throw new ChaoticKit.Exceptions.General.ImpossibleForkException()
                };
